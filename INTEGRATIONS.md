...
**labforge** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `labforge` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
labforge ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
labforge ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `labforge` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="labforge") for rec in records]   # records = your JSON output
